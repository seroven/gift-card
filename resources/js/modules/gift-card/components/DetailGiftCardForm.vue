    ...
    <Dialog header="Generar una Gift Card" v-model:visible="visible" modal :pt="this.$modalBlur"
        ...
        <div class="grid">
            ...
            <FormGroup label="Fecha de Emisión" :control="formGiftCard.controls.dateIssue" customClass="col-12 md:col-6">
                <Calendar v-model="formGiftCard.controls.dateIssue.value" dateFormat="dd/mm/yy"
            ...
            <FormGroup label="Fecha de Expiración" :control="formGiftCard.controls.dateExpired"
                ...
                <Calendar v-model="formGiftCard.controls.dateExpired.value" dateFormat="dd/mm/yy" :minDate="new Date()"
            ...
            <FormGroup label="Estado" :control="formGiftCard.controls.status" customClass="col-12 md:col-6">
                <Dropdown v-model="formGiftCard.controls.status.value" :options="status" optionLabel="description"
                    :class="`w-full-important ${formGiftCard.controls.status.getClass()}`"/>
            </FormGroup>
        </div>
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            this.loading = true;
            this.$axios.post('/api/gift-card/update/'+this.giftCardSelected.id,
                {
        ...
        setDefaultForm() {
            this.formGiftCard.matchValue({
                code: this.giftCardSelected.code,
                name: this.giftCardSelected.name,
                mount: parseFloat(this.giftCardSelected.mount),
                email: this.giftCardSelected.email,
                dateIssue: this.giftCardSelected.dateIssue,
                dateExpired: this.giftCardSelected.dateExpired,
                status: this.giftCardSelected.status,
            })
        }
    }
}
